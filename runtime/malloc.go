...
// Use of this source code is governed by a BSD-style -- governed:控制，管理
// license that can be found in the LICENSE file.

// Memory allocator, based on tcmalloc. -- allocator:分配器
// http://goog-perftools.sourceforge.net/doc/tcmalloc.html

// The main allocator works in runs of pages.
// Small allocation sizes (up to and including 32 kB) are -- 高达32KB
...
//	MHeap: the malloc heap, managed at page (4096-byte) granularity. -- granularity:粒度, 4kb/page
//	MSpan: a run of pages managed by the MHeap.
//	MCentral: a shared free list for a given size class.
//	MCache: a per-thread (in Go, per-P) cache for small objects.
//	MStats: allocation statistics.
//
// Allocating a small object proceeds up a hierarchy of caches: -- hierarchy:等级制度
//
//	1. Round the size up to one of the small size classes
//	   and look in the corresponding MCache free list. -- corresponding:符合的
//	   If the list is not empty, allocate an object from it.
//	   This can all be done without acquiring a lock.
//
//	2. If the MCache free list is empty, replenish it by -- replenish:填满
//	   taking a bunch of objects from the MCentral free list. --bunch:一批
//	   Moving a bunch amortizes the cost of acquiring the MCentral lock. -- amortizes:分期
//
...
// and s->needzero is set. There are two main benefits to delaying the -- benefits:好处
// zeroing this way:
//
//	1. stack frames allocated from the small object lists
//	   or the page heap can avoid zeroing altogether.
//	2. the cost of zeroing when reusing a small object is -- reusing:重复使用
//	   charged to the mutator, not the garbage collector. -- mutator:修改器
//
